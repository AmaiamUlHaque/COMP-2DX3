...
COMBINATION			EQU		2_1011	; this is the sequential combination expected - does NOT include the clock
...
PortM_Init 
		...
		BIC R0, R0, #0x07				;Modify the contents of R0 to clear bits PM0-PM2 without changing other bits (0=input)
		...
		;Pull-Down resistors for active high input
		LDR R1, =GPIO_PORTM_PDR_R
		LDR R0, [R1]
		ORR R0, R0, #0x07
		STR R0, [R1]
...
PortN_Init 
		...
		BIC R0, R0, #0x3				;Modify the contents of R0 to CLEAR bits PN0-PN1 without changing other bits (0 =INPUT)
		STR R0, [R1]					;Store what is in R0 into address pointed by R1 
