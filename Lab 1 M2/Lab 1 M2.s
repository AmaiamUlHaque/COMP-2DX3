...
PortF_Init 
		...
		;STEP 3: Set Port Direction (set bit 0 for output)
		...
		ORR R0, R0, #0x01				;Modify the contents of R0 to set bit 4 without changing other bits
		STR R0, [R1]					;Store what is in R0 into address pointed by R1 
		 
		;STEP 4: Enable Digital Functioning (set bit 0 for digital enable)
...
;Function PortM_Init
;Enable PortM and set bit PM0 for digital input
PortM_Init 
		;STEP 1 Activate clock (Set bit 11 in RCGCGPIO; In C pseudcode: SYSCTL_RCGCGPIO_R |= #0x20)
		 LDR R1, =SYSCTL_RCGCGPIO_R		;Stores the address of the RCGCGPIO register in R1
		 LDR R0, [R1]					;Dereferences R1 to put the contents of the RCGCGPIO register in R0
		...
		;STEP 3: Set Port Direction (CLEAR bit 0 for input)
		...
		AND R0, R0, #0xFE				;Modify the contents of R0 to clear bit 11 without changing other bits
		STR R0, [R1]					;Store what is in R0 into address pointed by R1 
		...
		ORR R0, R0, #0x01				;Modify the contents of R0 to clear bit 11 without changing other bits
		STR R0, [R1]					;Store what is in R0 into address pointed by R1 
...
Start 
		...
		;STEP 5: Outputting a Logic High to Port F Pin 0 (to switch on D4)   
LightOFF
		...
		BIC R0, R0, #0x01				;Modify the contents of R0 to set bit 0 without changing other bits
		STR R0, [R1]					;Store what is in R0 into address pointed by R1 

		;We now need to read the input (pin PM0), check if it is 0,
...
WaitForButtonPress 
		...
		LDR R0, [R1]
		ORR R0, R0, #0x01				;Clear bit 0 on Port F to switch off D4
		STR R0, [R1]
...
WaitForButtonRelease 
		...
		B LightOFF						;We only get here when button is released, so go back to beginning						;
