...
// Initialize onboard LEDs AND PN2 FOR OUTPUT
void PortN_Init(void){
	//Use PortN onboard LEDs
	SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R12;			// Activate clock for Port N
	while((SYSCTL_PRGPIO_R&SYSCTL_PRGPIO_R12) == 0){};		// Allow time for clock to stabilize
	GPIO_PORTN_DIR_R |= 0x07;        							// Make PN0 and PN1 out (PN0/PN1 built-in LEDs) AND PN2 OUTPUT
  ...
  GPIO_PORTN_AMSEL_R &= ~0x07;     							// Disable analog functionality on PN[0:1]
	FlashLED1(10);											// Flash LED D1 (Hello World)
	return;
}
...
void TIMER3A_IRQHandler(void){ 
	...
	//flash LED and output to a pin for 250ms and have a 25% duty cycle
	
	TIMER3_ICR_R = 0x1;								// Acknowledge Timer3 timeout
	GPIO_PORTN_DATA_R ^= 0b00000100; 	// output HI to PN2
	FlashLED2(25);										// flash LED D2 for 250ms
	GPIO_PORTN_DATA_R ^= 0b00000100;	// output LO to PN2
	SysTick_Wait10ms(75); 						// wait 750ms
