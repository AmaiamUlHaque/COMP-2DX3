...
uint32_t func_debug[80]; 											// array used to store realtime data 	

int main(void){
  				
	uint32_t 	delay = 1, 												  // this is a multiplier used for SysTick
	...
	while(count < 80){															// run program for fixed number of steps/samples
		GPIO_PORTN_DATA_R ^= 0b00000001;							// toggle LED for visualization of process
		SysTick_Wait10ms(delay);											// multipier*10ms
