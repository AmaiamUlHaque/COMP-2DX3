// Amaiam Ul Haque
// March 10, 2025
// COMP ENG 2DX3 - Lab 6 - Project 1 Deliverable
...
//initialise port M for buttons 2 & 3 (B2 = PM1 & B3 = PM0) as input
void PortM_Init(void){
	
	SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R11;                 // Activate the clock for Port M
	while((SYSCTL_PRGPIO_R & SYSCTL_PRGPIO_R11) == 0){};      // Allow time for clock to stabilize
		
	GPIO_PORTM_DIR_R = 0b00000000;       								      // Enable PM0-1 as inputs 
  GPIO_PORTM_DEN_R = 0b00000011;														// Enable PM0-1 as digital pins
	return;
}
...
void toggleStep(){ //toggles step angle between 11.25* & 45*
	uint32_t step1 = 16; 	//11.25*
	uint32_t step2 = 64; 	//45*
...
int main(void){
	...
	while (1){ 
		...
		//B3 - PM0 - RETURN HOME POSITION
		if ((GPIO_PORTM_DATA_R &= 0b00000001) == 0) {
			returnHome(); //waits 1s at home position to ensure returnHome is noticable
		}
